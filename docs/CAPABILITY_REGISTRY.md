...
- **Plan CAS verified 2026-09-19** Support repaired the client projection. `get_task_plan_kandev` returns metadata JSON in text block 1 and exact Markdown in block 2; use metadata `version` as opaque `expected_version` for whole-document replacement. Intentional large reduction also uses `allow_truncation=true`. Live compaction read back exactly, and an old-token write returned CONFLICT with unchanged body/version. Preserve exact preimages and all open records; see [verification receipt](cycle-archives/2026-09-19-plan-cas-verification.md). This is plan-write protection, not worker action fencing.
...
- **Bundled body receipt (verified 2026-09-08):** one existing queue row can contain several complete peer envelopes joined by blank lines. Recover every constituent sender prompt in order, rebuild the full wrapped body, and require exact byte count and SHA-256 agreement with the authoritative census before disposition. The last sender prompt alone is not the row. This does not authorize the Coordinator to combine distinct reports or discard an earlier question; reconcile each constituent independently.
- **Trigger** Every turn; act before the queue approaches its 15-entry limit.
- **Action** Call `get_message_queue_census_kandev`, proactively parallelize independent entries under A4, then let the primary refresh live task/session/provider state before verifying and acting on every result. After durable state is updated, pass only reviewed entries' exact `id` + opaque `claim` pairs to `dispose_message_queue_entries_kandev`. A `changed` result removes nothing: re-census, confirm the same immutable ID/provenance/hash is still the reviewed item, and retry it with the new claim. Treat `not_found` as an explicit concurrent-disposition result, never as proof that this call removed the row.
- **Authority** Human-authorized queue triage; the primary retains all mutation and reporting responsibility.
- **Capability** **VERIFIED WORKING in the live task-runtime surface (2026-09-05):** `get_message_queue_census_kandev` returns ordered immutable entry IDs, opaque claims, safe provenance, hashes, sizes, and the before count; `dispose_message_queue_entries_kandev` returns per-entry `removed`, `changed`, or `not_found` plus atomic before/after counts. The canonical upstream/coalescing implementation remains owned by task `ca015838-e5cf-4294-b3bb-9c50576a5fe6` / https://github.com/kdlbs/kandev/pull/3377 until merged and deployed.
- **Evidence** Before/after ordered IDs and counts, helper receipts, primary action receipts, and exact per-entry disposition results. Live acceptance removed exact reviewed entries while preserving a concurrent unread Human row; a later concurrent queue reorder returned `changed`, fresh census claims then removed only the original three reviewed IDs and retained the unrelated row.
...
- **Residual degradation (2026-09-05):** The guarded census/disposition portion is callable, but canonical identical-routine-wake coalescing and upstream deployment are not thereby proved. Continue to preserve each non-identical Human/task/peer entry, and keep the exact upstream owner above until its complete reviewed trigger clears.
- **Upstream repair owner:** canonical Kandev task `ca015838-e5cf-4294-b3bb-9c50576a5fe6`, “Add guarded queue claim and routine wake coalescing”, and https://github.com/kdlbs/kandev/pull/3377 own durable upstream delivery. Acceptance binds every disposition to an immutable row identity, preserves FIFO/restart durability and cross-workspace authorization, exposes before/after ordered counts, and coalesces only identical pending routine wakes for the same target—not Human input, distinct task/peer reports, or materially different payloads.
- **Confirmed routing defect (Support request `9173dd30-1b9b-412f-b597-0f52a4a1b28c`, 2026-09-03):** repeated routine carriers reached a stale analysis-only Coordinator session even though the canonical current Coordinator retained full board authority. This is not global permission loss. The existing repair must also bind execution to the canonical same-workspace Coordinator identity and current authorized scope. Acceptance requires one representative `WAKE:CYCLE` to reach that Coordinator exactly once; no disposable carrier may select a stale/restricted session, and a genuinely restricted target must return an explicit fail-closed scope receipt. Until the reviewed Host repair is deployed, human-directed current-session board management remains available and routine refusals are a routing degradation, not evidence that the Coordinator role lost authority.
...
- **Nonterminal predecessor fallback (verified 2026-09-08):** a Human-authorized, safely contactable same-task `WAITING_FOR_INPUT` predecessor can return its own exact queue census in one bounded handoff, then park. Verify the original census tool result in its transcript; preserve rows and keep the successor the sole plan/board writer. Do not force retirement to qualify for the terminal reader. `is_current` is caller-relative; `is_primary` identifies the primary across callers. See the linked recovery runbook.
...
- **Action** First prove the failed session still exists; recover before any session deletion because deletion transactionally purges its queue. Bind to the exact calling task and exact terminal session. Page `get_terminal_session_message_queue_kandev` by opaque cursor, verify one stable snapshot hash plus every complete-body SHA-256 and FIFO position, then repeat a fresh snapshot read to prove the source did not change. Reconcile every entry FIFO against the live plan, board, sessions, and provider state. Preserve the source queue; the reader has no acknowledgement, reservation, replay, removal, or mutation authority. If the session is already absent, use only an already-verified retained artifact or reconstruct an explicitly incomplete record from durable plan/conversation/provider state; do not retry queue reads or ask Support for deleted bodies. If the direct surface is absent, ask Support to repair/provision that reusable capability, not to read or relay the queue.
- **Capability** **VERIFIED WORKING (task-runtime Coordinator, 2026-09-04):** `get_terminal_session_message_queue_kandev({session_id, cursor?, max_body_bytes?})`; exact session must belong to the server-injected caller task and be `COMPLETED`, `FAILED`, or `CANCELLED`; body-page limit is 1–256 KiB and attachments/metadata are excluded. Procedure: [failed-session queue recovery](RUNBOOK.md#recover-unread-messages-from-a-failed-coordinator-session).
...
- **Evidence** Support request `825e8aff-5625-483b-9f44-7e11d5ca6b78` provisioned the reusable surface. Acceptance against cancelled session `5cca265d-658c-4648-b165-d862a8d48e01` reconstructed 9 FIFO entries / 35,905 UTF-8 bytes in 36 one-KiB pages; every computed body SHA-256 matched, all pages shared snapshot `9f51da7fa3ceb9535db030758fa1946b5d167599eee2b8220a8e7430a62943b1`, and an immediate 256-KiB reread was byte-identical. A call against live session `9eae93b7-ca39-47f0-afd0-9e0e054d07d8` failed closed with `CONFLICT`. Exact task/session identities, ordered IDs/positions, per-entry hashes, stable snapshot hash, and repeat-read equality prove `queue_mutated=false`.
...
- **2026-09-20 guarded handoff catalog update:** `handoff_coordinator_primary_kandev` is now exposed, but successful execution is not verified. It requires an idle bootstrapped sibling, exact profile/model with `model_verified=true`, both queue incarnation IDs, and an idempotent operation ID. The current session-list response provides incarnation IDs but omits strict model-attestation fields; ordinary spawn has no strict-launch option. Conversation `metadata.model=gpt-6-astra` proves the candidate's runtime model, not the missing strict launch receipt. Both predecessor and candidate queue censuses still return `UNKNOWN_ACTION`. Preserve the candidate and sole predecessor; do not claim queue/routine transfer or bypass the attestation requirement. Support `18a6dfc0-8edb-44f6-a4b8-aa861053e051` owns this reusable capability gap; consume its pushed result without polling. This supersedes historical catalog-absence claims only, not the acceptance gates.
- **Partial recovery observed 2026-09-15:** the Human confirmed manually promoting the successor; no agent invoked a promotion transaction. Queue census still returned `UNKNOWN_ACTION`, and a routine wake subsequently reached the old non-primary session. Primary readback proves the current role owner, not queue parity, routine targeting, or fencing; preserve the predecessor and keep those criteria open. Future rotations must be automatic; do not ask the Human to repeat manual promotion.
...
- **Action** The task's own active session calls `add_branch_to_task_kandev`. On the first call it may identify the repository by URL/ID and omit `checkout_branch` to generate a branch. Treat the returned `checkout_branch`, `worktree_branch`, row ID, worktree path, task root, and `agent_cwd_changed` as one identity receipt. For an exact retry, call from that same owning task/session with `repository_id`, `base_branch`, and the returned `checkout_branch`; the server resolves the existing repository-position/environment entry instead of creating another row or worktree. Verify the response and then run `pwd`, `git status --short --branch`, `git rev-parse HEAD`, and the intended base-ref check before implementation.
- **Capability** **VERIFIED WORKING (2026-09-05):** `add_branch_to_task_kandev`; deployed backend `v0.93.0-97-gd0ce9383d`. Provider repositories are cloned through the authenticated task/session cloner, caller provenance is server-authored and same-task/active-session checked, generated branches are projected into both branch fields, and ambiguous retries fail closed.
- **Evidence** Support repairs `808d887b-c78f-4928-88a8-cdd84ae2e56c` and `0e2631fb-4017-4ad8-898f-758c498c2133`; owning-session acceptance on task `8f8a784d-92ea-421f-a368-154ef915fe4e` reused row `8010334c-d571-4979-834e-d22a4da689d5` and worktree `/data/tasks/register-task-runtim_qfb1v4eh/yattdev-kandev-service-main`, returned `checkout_branch=worktree_branch=feature/register-task-runtim-k1g`, and proved a clean `HEAD == origin/main == aeb9f58582d572a6ef36b987ffe2a9d62b0e6a0c` without duplicate materialization.
- **Never** Invoke the retry cross-task, manually clone as a fallback, use `add_workspace_sources_kandev` during an active turn, infer identity from a stored empty generation instruction, accept mismatched branch/path/row fields, or continue implementation before the Git receipt passes.
...
- **Observed scope limit (2026-09-09):** `remove_task_dependency_kandev` rejected same-workspace dependent IDs with `task_id must be your current task`. Read access does not imply mutation scope. Preserve both endpoint readbacks; a Done-only obsolete edge is housekeeping, not unfinished delivery. Do not wake a parked Done session or use a database workaround. Route reusable cross-task relation capability through its existing owner.
...
- **Action** DECIDE every in-scope concrete action; RECOMMEND when multiple in-scope options remain; ESCALATE only a security/trust-boundary decision or material scope change.
...
- **Action** Treat draft as routine provider metadata and a creation default, not a hold until Human-QA. Evaluate readiness immediately and on every task touch. The task agent owns missing implementation/evidence; once the complete receipt is independently verified, the Coordinator may perform the mechanical ready transition. Strict universal order: repair/rerun required pipeline issues → execute ready transition → verify provider readback is non-draft at the unchanged head → refresh post-ready gates → require every required current-head job to be terminal without failure → notify the reviewer. Human-QA blocks readiness only for a specific remaining manual/visual acceptance need.
- **Capability** [Turn a draft PR/MR ready through its task agent](RUNBOOK.md#turn-a-draft-prmr-ready-through-its-task-agent).
- **Evidence** Canonical PR URL + exact clean pushed upstream-matched head; fresh exact-head CI census before and after the ready transition (including newly triggered `pull_request` jobs) with every required job terminal without failure before reviewer contact; zero unresolved threads; visual/manual evidence only when the change requires it. A repository-policy gate requiring maintainer or architecture approval needs a substantive response authored by that named authority; the contributor's own issue, PR body, checklist, or recommendation is only a proposal.
- **Never** Ask for confirmation merely because the PR is draft, hold every draft until Human-QA, or send any review request/mention/assignment/notification while provider state is draft or a required current-head job is failed, pending, or unclassified. Never count a self-authored proposal as external approval. Never notify from the draft-era check snapshot. Never rebase, deploy, treat ready-for-review as acceptance, or become the implementer. Merge only under the exact named-program Human grant in I3 after its complete gate.
...
- **Scoped Human override (2026-09-16):** Coordinator-plugin chat may open a draft before maintainer feedback and refine afterward; inspect/reuse the existing RPC pattern where applicable. Pending discussion must remain explicit, and readiness/merge gates still apply. See [decision](DECISIONS.md#coordinator-plugin-chat-drafts-and-existing-rpc-patterns-2026-09-16).
...
- **Redirected stdin and runtime-wrapper ownership repaired; persisted-model replay ports and Support execution remain degraded (verified 2026-09-02)** Deployment-local commit `ab8174cea69f0e503439d06340abed5d19716e5a` repaired framed, binary-safe stdin for guarded `docker compose exec -T`; exact task acceptance delivered 3,932,160 bytes with matching SHA-256 `083c9bfa3894b7f7ca25dfe0a8fabe667e9e98aacc4109f15f9d95c823655f37` and passed empty stdin. Commits `5bd2af710676b07ad10e47a86655d53adaefdb80` and `5e30d760393c23fbc39a4ab8f0ecad916134e8d0` repaired saved-model replay and runtime-wrapper ownership. Commit `9481f71` forwarded the allowed port environment to Compose execution, but the unchanged exact task sequence still returns `invalid IP:0` instead of saved ports 53403/60003. Support requests `ff6ba1b6-2bac-4b1b-ae5b-c31546671cd3`, `266d834e-c3ec-4763-9e83-d797e4d21baa`, and `f83d9473-113d-4dc6-ba8e-807a1f73ca25` then each returned terminal `BLOCKED` while explicitly naming no external dependency and performing none of the authorized synthetic reproduction or repair. After three identical execution failures, do not submit another duplicate Support request: preserve the exact runtime, volumes and data, and escalate repair/restart/rerouting of the Support execution worker to the Human. Resume with one fresh Support continuation only after that capability is restored. Only exact ports plus localhost/LAN acceptance closes the degradation.
...
- **Same-workspace service handoff (human-directed 2026-09-02)** A task-owned application/runtime may be consumed by another exact task in the same workspace through a reviewed reachable service endpoint and disposable scoped credentials. Read back both full task UUIDs and workspace IDs, preserve provider ownership, and verify service health plus consumer-side reachability. Do not share the provider's raw SQL, volume, filesystem, logs, worktree, or unrelated credentials. A platform-enforced inability for the authorized consumer to reach the provider is a capability defect, not expected task isolation.
...
- **Current Codex execution path (2026-09-08):** the Codex 0.147 `unified_exec=false` workaround below is superseded. Under 0.153.4 it selected a legacy 30-second synchronous path without a resumable handle. Support request `5b3817ec-da73-4e6f-bca1-e27d6141a934` removed that override in deployment commit `06d5375b66026cc01687b02d0a2f404d1b2d63d3`. Owning Coordinator acceptance passed: `exec_command(tty:true, yield_time_ms:1000)` returned session `64075`, with stdin/stdout TTY assertions true; `write_stdin` with Ctrl-C to that exact session returned captured `KeyboardInterrupt` and terminal exit 1. A completed non-TTY command returned both output markers and its intended exit 7. Use the execution handle for cancellation and result collection; poll with empty input only while still running. Never use a host PID or restore the obsolete override. This verifies this caller's execution path; it does not discharge separate credentialed-TTY integration requirements on other tasks.
...
- Security or trust-boundary decisions: secret/credential disclosure or permission expansion, authentication/authorization weakening, security-policy bypass, cross-workspace or trust-boundary access.
- Material scope changes: changes to the approved objective, public contract, architecture, schema, external integration, or delivery boundary.
- Destructive or practically irreversible operations are Coordinator-decidable inside approved scope only after exact ownership, preservation, containment, and blast-radius proof. Removing unique/still-needed state or changing the delivery boundary is a scope change and must be escalated.

### I2. Explicitly NOT a second approval gate
Production, protected/release branch, cost, and external-communication **labels** do not create a second approval principal. Routine task-related upstream comments, questions, and reviewer notifications have standing Human authorization. Decide every in-scope action and escalate only a security/trust-boundary decision or material scope change. Verified redundant task-local worktree/branch cleanup is Coordinator-approved ([C2](#c2-destructive-filesystem-action-on-another-tasks-resources)) and must not be escalated.

### I3. Structural boundaries that never bend
- **Merging (default)**: no agent merges. Green/clean/approved = ready for the repository owner, not authorization.
- **Named-program merge override (Human-directed 2026-09-04)**: the canonical Kandev Coordinator may merge Coordinator-plugin and Redmine program PRs when authenticated provider permission and repository policy allow it, but only after unchanged clean/pushed head identity, independent Review, distinct QA, terminal required checks, zero actionable threads, and every applicable security/product/visual/runtime gate. Use the supported non-rewriting method and verify accepted head plus merged commit. This grant excludes force-push, history rewriting, unique-state deletion, credential scope expansion/disclosure, security bypass, cross-workspace access, production deployment, and release. Without provider permission, the upstream maintainer remains the owner.
- **Upstream `kdlbs/*` PR that is READY**: **Coordinator action, not a dead end** — post a mention of `@carlosflorencio` (maintainer, holds the merge) on the PR; add `@jcfs`/`@zeval` only when prior authorship on the touched paths shows they own that area. Before contact, verify the unchanged exact head is non-draft and mergeable, threads are resolved, and every required current-head pipeline job is terminal without failure; repair/rerun any failed, pending, or unclassified result first. **Once per head**, recorded with PR number + head SHA; re-notify on a new push, never on an unchanged head. Agents are credential-blocked (D18), so the Coordinator posts it.
...
### J7. Workflow-configured models while fixed lane gates are suspended
- **Startup denial visibility verified (2026-09-08; historical):** `list_task_sessions_kandev` returns the bounded `startup_failure` for a strict pre-inference model rejection. The canonical Coordinator verified `exact_model_unavailable`, requested `gpt-5.4`, reason `requested_not_advertised`, and both `inference_started=false` and `substitution_performed=false`. Source `22a6632c0a02d7c566ea19690330dfc0a504d03b` recognizes the exact persisted ACP wrapper. The 2026-09-14 Human policy change supplies the previously awaited trigger: former fixed-model availability no longer blocks current launches through the workflow-configured profile.
...
- **Trigger** A board audit, task start/resume/message, or lane move exposes an assignee, start failure, or nonterminal session whose model differs from the former fixed lane map.
- **Action** Treat the current workflow/workstep-configured Codex or Copilot profile as authoritative. Record the executing host, profile, and effective model as provenance, but do not block, park, replace, or reroute a task solely for a former lane-model mismatch. Reclassify model-only blockers and resume through the configured workflow when non-model prerequisites pass. Keep independence, exact-head, provider, lane, and single-owner gates unchanged.
...
- **Authority** Human suspension of per-workstep model gates 2026-09-14; see [Use workflow-configured models while fixed lane gates are suspended](RUNBOOK.md#use-workflow-configured-models-while-fixed-lane-gates-are-suspended).
- **Evidence** Exact task, physical lane, configured profile, session ID, executing host, effective model, active/parked classification, and successful owner/session readback after any launch.
- **Escalate to** The Human only when the workflow configuration itself requires an author decision; ordinary launch failures remain operational recovery work.
- **Never** Recreate the former lane map, override the Human-maintained workflow/workstep configuration, or preserve a model-only blocker after this suspension.
