...
- **Redirected stdin repaired; port publication still degraded (verified 2026-09-02)** Deployment-local commit `ab8174cea69f0e503439d06340abed5d19716e5a` repaired framed, binary-safe stdin for guarded `docker compose exec -T`. Exact task acceptance in `1f434680-0901-4a0c-abaf-1c48d050f7d4` delivered a 3,932,160-byte binary payload with matching local/container SHA-256 `083c9bfa3894b7f7ca25dfe0a8fabe667e9e98aacc4109f15f9d95c823655f37`, propagated pipeline exit 0, and passed explicit empty stdin. Port publication remains defective: after proving the exact saved model, named volumes/network, config hashes, and rendered mappings, an authorized `up -d --force-recreate db web` created fresh healthy/running containers whose publishers were still `PublishedPort:0`; both `compose port` calls returned `invalid IP:0`. Preserve that exact runtime and data, do not claim an external handoff reachable until localhost/LAN probes pass, and never infer runtime publication from rendered ports. Follow-up Support request `f1295766-fc21-4639-970b-5dc8365a0012` owns the port-only safe-superset repair; do not duplicate it.
