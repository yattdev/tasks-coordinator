...
- **Status** **PARTIALLY BLOCKED — raw Docker still fails closed, but destructive Compose project selection is unsafe across sibling/disposable directories until Support request `a9328e69-e01d-4d8e-8cbc-d18e41c3cdcd` is resolved and deployed.**
- **Action** Non-destructive `docker compose config` may still verify interpolation. Do not run `down`, `rm`, `kill`, `--volumes`, or another destructive/mutating cleanup from a sibling clone or disposable project directory: preserve the reproduction and route the root incident to the deployment-local guard owner. Follow [Verify task-scoped Compose isolation](RUNBOOK.md#verify-task-scoped-compose-isolation-from-a-guarded-task-session), including its temporary destructive-operation hold.
...
- **Command-scoped interpolation (verified 2026-08-31)** The guard transports only `COMPOSE_PROJECT_NAME`, `DB_PORT`, and `WEB_PORT` into Compose interpolation, but project isolation remains broker-owned: the authoritative project is the derived `kd_<task-hash>` identity, not an arbitrary caller-selected name. Use that derived value in task commands. Valid values rendered exact isolated ports `13306`/`18080`; an unrelated sentinel remained absent, an out-of-range port failed closed with exit 78, and direct `docker ps` remained denied with exit 78. Deployment repair: `yattdev/kandev-service` commit `984a27fbdc8124d751bfdde3b8e67f2459c78595`; this is deployment policy, not an upstream `kdlbs/kandev` source change.
