...
### J7. A host-specific task is assigned to or retains the wrong lane model
...
- **Trigger** A board audit, task start/resume/message, or lane move exposes an assignee or nonterminal session whose authoritative host/client and resolved model do not match that host's lane policy.
- **Action** Resolve the host/client family first. For Codex-hosted execution enforce: Coordinator=`gpt-5.6-sol` OR `gpt-6-astra`; Spec/QA=`gpt-5.6-sol`; Work/Blocked/Human-QA=`gpt-5.6-terra`; Review=`gpt-5.5`; PR/Done=`gpt-5.4`; CI Fixup=`gpt-5.6-luna`. Both Sol and Astra are authorized for the permanent Coordinator and replacement primaries; Astra remains reserved for that role and audit helpers explicitly retain Sol rather than inheriting Astra. Backlogs, Todo, and ToDeploy do not start task Codex sessions; the Coordinator is the explicit Backlogs exception. Copilot-hosted and Claude-hosted sessions use their configured supported runtime selection in every lane: never impose a Codex workstep mapping or hard model-mismatch gate on them. Do not start or resume a true Codex host-specific mismatch; select and verify a fresh correctly mapped profile. Distinguish active `RUNNING`/`STARTING` use from parked `WAITING_FOR_INPUT`/`CREATED` risk.
...
- **Authority** Human cost-control directive 2026-09-06, Coordinator Sol-or-Astra clarification 2026-09-08, and Codex-only workstep-gate clarification 2026-09-11; see [Audit and contain Codex lane-model mismatches](RUNBOOK.md#audit-and-contain-codex-lane-model-mismatches).
- **Evidence** Exact task, physical lane, session and profile IDs, authoritative host/client family, resolved model, active/parked classification, and fresh correctly mapped effective-profile readback after any replacement.
...
- **Never** Infer the host/client from a model string or profile name, impose any Codex lane model rule on Copilot-hosted or Claude-hosted work, park/block/replace either host solely for its configured or dynamically selected model, wake a truly mismatched parked Codex session to ask it to switch, substitute a cheaper or heavier Codex model for the configured Codex lane, or count terminal history as current usage.
