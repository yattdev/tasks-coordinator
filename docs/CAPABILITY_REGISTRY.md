...
- **Status** **PARTIALLY VERIFIED — exact explicit `kd_` project/clone binding now prevents sibling cleanup from selecting a protected project, but the required denial/correlation audit remains incomplete under Support follow-up `d9094d17-0741-49ee-bb3c-579812889542`.**
- **Action** Non-destructive `docker compose config` may verify interpolation. Keep the general hold on `down`, `rm`, `kill`, `--volumes`, and other destructive/mutating cleanup from sibling/disposable directories until the follow-up audit contract is deployed and verified. A bounded two-synthetic-project acceptance fixture may exercise the exact bound identities under the linked runbook; never use the damaged task as the protected target.
...
- **Audit follow-up (open 2026-09-01)** The deployed broker records authorized destructive `attempt/ok|error` events with task root, cwd, project, and command but emits no audit ID or ownership decision, does not audit a destructive request denied during project/model ownership loading, and returns no correlation ID to the caller. This leaves the approved fail-closed audit contract incomplete even though the isolation behavior passes. Follow-up `d9094d17-0741-49ee-bb3c-579812889542` owns the deployment repair; do not duplicate or poll it.
