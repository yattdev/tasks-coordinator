...
- **Active stdin/port degradation (observed 2026-09-02)** In task `1f434680-0901-4a0c-abaf-1c48d050f7d4`, guarded `docker compose exec -T ... < file` returned exit 0 while delivering zero bytes, and guarded `up` returned 0 from a rendered model containing explicit allowed ports while creating no host mappings; `compose port` returned `invalid IP:0`. Internal service/API checks passed, proving these are transport/publication defects rather than application failures. Preserve the exact runtime and use an approved in-container read-only file bind only when the task plan explicitly authorizes that one restore; do not call an external handoff reachable until host/LAN probes pass. Support request `5d2dc347-f2d4-4273-969c-6fd94e0944c6` owns the safe-superset repair. Never treat exit 0 as stdin-delivery evidence or rendered ports as runtime publication evidence.
