...
- **Redirected stdin repaired; force-recreate publication still blocked by persisted clone ownership (verified 2026-09-02)** Deployment-local commit `ab8174cea69f0e503439d06340abed5d19716e5a` repaired framed, binary-safe stdin for guarded `docker compose exec -T`. Exact task acceptance in `1f434680-0901-4a0c-abaf-1c48d050f7d4` delivered a 3,932,160-byte binary payload with matching local/container SHA-256 `083c9bfa3894b7f7ca25dfe0a8fabe667e9e98aacc4109f15f9d95c823655f37`, propagated pipeline exit 0, and passed explicit empty stdin. The first port repair `5bd2af710676b07ad10e47a86655d53adaefdb80` makes force recreation load the saved task/clone-bound model, but exact task acceptance then failed before Docker with exit 78, `Compose project is already owned by a different task clone`, even though the same canonical task/root/runtime/project had previously operated it. Preserve the exact runtime, volumes and data; do not select another project/clone or claim an external handoff reachable. Support request `9134cd16-2fd8-4f84-9b31-73977c0b5634` owns the persisted-ownership repair and must retain fail-closed sibling/cross-task denials. Only exact port plus localhost/LAN acceptance closes this degradation.
