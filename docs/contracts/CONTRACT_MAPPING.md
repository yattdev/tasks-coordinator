...
Status: **agreed 2026-09-03; contract 1.2.0 hardened 2026-09-07**, implementing `docs/DECISIONS.md#coordinator-policy-is-contract-validated-not-hand-copied-2026-09-03-human-directed` and the later machine-gate decision.
...
## Retrieval-only source relocation (2026-09-19)

Detailed source sections now live verbatim in `docs/OPERATING_POLICY.md`;
`PROMPT.md` is the canonical compact kernel and retrieval index. Contract 1.2.0
retains its historical 2026-09-07f provenance and digest: this docs-only move
does not claim the older plugin snapshot implements the newer routing policy.
A future semantic contract update must be reviewed and re-vendored normally.
...
| --- | --- | --- |
| `source_charter_effective_version` | Historical charter revision whose mapped semantics produced this contract, preserved in Git history | Contract 1.2.0 records 2026-09-07f; this is historical provenance, not a claim of parity with every newer live policy. A future semantic contract release records the then-current policy generation and follows normal review/re-vendoring. Retrieval-only relocation does not change the JSON or digest. |
...
| `authority_boundaries.approval_principal`, `.scope`, `.cross_workspace_authority` | `docs/OPERATING_POLICY.md` "FULL COORDINATOR APPROVAL AUTHORITY" section (same-workspace approval principal); `docs/FILESYSTEM_DOCKER_CONTRACT.md` §3 (workspace-scoped authority, peers have no standing over each other) | Cross-workspace authority is always `false`; no evolution may set it `true`. `cross_workspace_authority` must be **present** and `false` — a contract that removes the key entirely must fail closed the same as one that weakens it to `true` (the validator checks presence explicitly, not just `!= true`, since a naive default-on-missing-key check would otherwise let removal silently pass; see `fixtures/missing_cross_workspace_authority_contract.json`). `scope` must stay exactly `"same_workspace_only"` — weakening or removing it is independently enforced even though `approval_principal`/`cross_workspace_authority` may still check out individually; see `fixtures/weakened_authority_scope_contract.json` and `fixtures/missing_authority_scope_contract.json`. `approval_principal` must stay exactly `"coordinator"` — weakening it to `"none"` removes the accountable principal for every `coordinator_decidable_examples` entry this same object lists, even though `scope`/`cross_workspace_authority` stay correct; enforced by the validator's `missing_required_invariant` check, see `fixtures/false_approval_principal_none_contract.json`. |
| `authority_boundaries.coordinator_decidable_examples` | `docs/OPERATING_POLICY.md` "Everything else is Coordinator-decidable" paragraph | Additive (new examples) is a **minor** change; removing an example without a corresponding Human-reserved reclassification is **breaking**. |
| `authority_boundaries.human_reserved_classes` | `docs/OPERATING_POLICY.md` "The only approval classes reserved for the Human are (1)... (2)..." | These two classes (`destructive_or_irreversible`, `security_or_trust_boundary`) are the floor. A contract or overlay may only add to this set, never remove from it — enforced by the validator's `missing_required_invariant` / `overlay_widens_authority` checks. |
| `workspace_lane_ownership.*` | `docs/OPERATING_POLICY.md` "Monitor every task in spec, work, review, qa, pr, ci-fixup, AND Done"; `docs/FILESYSTEM_DOCKER_CONTRACT.md` §3 (peer workspaces) | `done_is_terminal_integrity_lane` must stay `true` — Done is never an ignored archive — enforced by the validator's `missing_required_invariant` check; see `fixtures/false_done_terminal_integrity_lane_contract.json`. `monitored_lanes` must keep `done` — a `done_is_terminal_integrity_lane: true` claim about a lane that isn't even monitored is meaningless; see `fixtures/missing_done_monitored_lane_contract.json`. `unit` must stay exactly `"workspace"` — this is the granularity every other `workspace_lane_ownership`/`authority_boundaries` field assumes; see `fixtures/weakened_lane_unit_contract.json` and `fixtures/missing_lane_unit_contract.json`. `peer_model` must be **present** and `true` — workspaces have no standing over each other (`docs/FILESYSTEM_DOCKER_CONTRACT.md` §3); see `fixtures/weakened_peer_model_contract.json` and `fixtures/missing_peer_model_contract.json`. `cross_workspace_standing` must be **present** and `false` — the lane-ownership-level twin of `authority_boundaries.cross_workspace_authority`; see `fixtures/weakened_cross_workspace_standing_contract.json` and `fixtures/missing_cross_workspace_standing_contract.json`. `auto_start_lane_move_requires_settled_lifecycle` must stay `true` — an auto-start lane move must never fire against a task whose lifecycle has not yet settled; see `fixtures/weakened_auto_start_lane_move_contract.json` and `fixtures/missing_auto_start_lane_move_contract.json`. |
| `queue_claim_identity.*` | `docs/OPERATING_POLICY.md` "Queue identity and disposition are per-entry..." paragraph (queue helpers/coalescing section); `docs/DECISIONS.md#cross-sender-routine-wake-coalescing-is-identity-based-not-sender-based-2026-09-03` for the routine-identity fields specifically | `audit_model` must stay `exact_entry_never_global_watermark`; this is the specific defect class the 2026-09-01j decision closed. `coalescing_rule` must stay exactly `only_identity_equivalent_pending_routine_wakes_for_same_target` — a broader rule (e.g. `all_messages`) would coalesce distinct Human/task/peer messages, which `coalescing_forbidden_for` exists to prevent; enforced by the validator's `missing_required_invariant` check, see `fixtures/weakened_coalescing_rule_contract.json`. `minimum_trusted_envelope` must keep `entry_id` (see `fixtures/missing_entry_id_envelope_contract.json`) and `workspace_id` (see `fixtures/missing_workspace_id_envelope_contract.json` — without it a claim's trusted envelope cannot be scoped to one workspace, reopening the cross-workspace-authority hole at the queue-claim layer) and `coalescing_forbidden_for` must keep `human_input` (see `fixtures/missing_human_input_coalescing_forbidden_contract.json`) — all independently enforced floors, not implied by `audit_model`/`coalescing_rule` alone. `claim_collision_check` must stay `deterministic_claim_set`, never `"none"` — without an actual collision check, two claims against the same resource key could both be granted concurrently, which is exactly the scale RFC's "zero claim overlap" invariant; see `fixtures/false_claim_collision_check_none_contract.json`. Added at `contract_version` `1.1.0` (additive/MINOR): `routine_identity_components` must be a superset of `{workspace_id, routine_type_or_name, policy_or_prompt_version_generation, semantic_scope_generation}` — this is the canonical Host routine identity, deliberately independent of sender task/session/message ID; see `fixtures/missing_semantic_scope_generation_routine_identity_contract.json`. `routine_identity_excludes_sender_ids` must stay `true` — see `fixtures/false_routine_identity_excludes_sender_contract.json`. `cross_sender_coalescing_permitted` must stay `true` — without it, an identical generation delivered by two different senders would never coalesce, defeating the point of a sender-independent identity; see `fixtures/false_cross_sender_coalescing_permitted_contract.json`. `coalescing_preserved_state` must stay exactly `single_pending_successor_or_freshness_bit` — coalescing must never "drop" the sole effective wake outright; see `fixtures/weakened_coalescing_preserved_state_contract.json`. |
| `worker_helper_receipts.*` | `docs/OPERATING_POLICY.md` "helpers never mutate the board, provider, task worktrees..." paragraph; "A helper receipt never proves that its source queue row was claimed..."; `docs/rfcs/PLUGIN_SCALE_RFC.md` §2.5 (single-writer mutation lane) | `workers_never_mutate` is a floor invariant shared with the scale RFC's read-only worker pool design. `mutation_serialized_by` must stay exactly `"single_writer_lane"` — this is what actually serializes the mutations `workers_never_mutate` forbids workers from making; weakening or removing it would leave "workers never mutate" true in isolation with no named serializing lane; see `fixtures/weakened_mutation_serialized_by_contract.json` and `fixtures/missing_mutation_serialized_by_contract.json`. `receipt_is_not_proof_of` must keep all four disposition classes (`queue_row_claimed`, `queue_row_acknowledged`, `queue_row_removed`, `capacity_released`) — dropping any one would let a receipt be silently treated as sufficient for the dropped disposition; see `fixtures/weakened_receipt_is_not_proof_of_contract.json` (one entry removed) and `fixtures/missing_receipt_is_not_proof_of_contract.json` (whole field removed). `receipt_required_fields` must keep `claim_or_lease_id` — without it a receipt cannot be tied back to the exact claim it attests to; see `fixtures/missing_claim_or_lease_id_receipt_contract.json`. `freshness_barrier_required_before_reporting` must stay `true` — without it a worker could report against state it read before a concurrent mutation, indistinguishable from a fresh report; see `fixtures/false_freshness_barrier_contract.json`. Added at `contract_version` `1.1.0` (additive/MINOR): `routine_wake_coalescing_receipt_fields` names the full schema for a coalesced routine-wake receipt (`canonical_entry_id`, `absorbed_source_entry_ids`, `absorbed_source_entry_count`, `absorbed_source_entry_timestamps_without_bodies` — never the absorbed entries' bodies — `leader_fencing_token`, `dirty_generation`, `post_run_requeue_required`); the validator enforces the safety-critical subset `{canonical_entry_id, absorbed_source_entry_ids, leader_fencing_token, dirty_generation}` — see `fixtures/missing_leader_fencing_token_routine_wake_receipt_contract.json`. |
| `gates.review`, `gates.qa`, `gates.readiness`, `gates.done_integrity` | `docs/OPERATING_POLICY.md` "exact-head Review/QA/CI readiness" (multiple sections); "DONE TERMINAL-INTEGRITY GATE" | All four must keep `exact_head_required: true`. This is the single most load-bearing invariant in the contract — it is what stops a stale-head approval. `gates.review` and `gates.qa` must additionally keep `independent_session_required: true` (symmetric requirement, both checked identically by the validator) — self-attestation by the authoring session is not a gate; see `fixtures/false_review_independent_session_contract.json` and `fixtures/false_qa_independent_session_contract.json`. `gates.readiness.recheck_after_draft_to_ready_transition` must stay `true` — readiness must be rechecked after a draft-to-ready transition, not only evaluated once while still draft; see `fixtures/false_readiness_recheck_contract.json`. `gates.done_integrity.terminal_receipt_required` must stay `true`, independently of `exact_head_required` — without it, a matching head alone could reach Done with no terminal receipt at all; see `fixtures/false_done_terminal_receipt_required_contract.json`. |
| `task_evidence_gates.*` | `docs/OPERATING_POLICY.md` "UNIVERSAL TASK-TOUCH CHECKLIST" items 7–9 and "MACHINE-ENFORCED CYCLE AND TRANSITION EXIT GATES" G3–G5/G10 | Contract 1.2.0 floor. Fresh status barriers, owner-first anomaly context, all-named-surface absence proof, current-cycle blocker falsification, and remote/provider delivery containment are all required. `delivery_containment_proof` is a required superset of task head, named remote reachability, canonical delivery identity, and provider-proven merge/release state. Removing any one reopens the exact false-deployable incident this version closes. |
| `workflow_transition_gates.*` | `docs/OPERATING_POLICY.md` G6–G7; `TASK_MONITORING_CHECKLIST.md` transition matrix | Contract 1.2.0 floor. Both pre- and post-transition receipts are mandatory; head changes invalidate prior gates; Review and QA need explicit verdicts; ToDeploy needs provider-proven merge/release; Done needs its terminal receipt; and a failed postcondition never counts as advancement. |
| `cycle_exit_gates.*` | `docs/OPERATING_POLICY.md` G1–G2/G8–G9 and WAKE:CYCLE finish contract | Contract 1.2.0 floor. Live/open task-ID set equality, complete accountable entries, current-cycle Blocked records, mutation readback, holding-task disposition, persisted-state readback, and fail-closed cycle completion are independently enforced. `every_open_entry_requires` must retain owner, health, cycle check, last/next action, trigger, and fallback. |
| `continuity_state.*` | `docs/OPERATING_POLICY.md` "PERSISTED STATE"; `CONTINUITY.md`; `RUNBOOK.md` state-plan hygiene | Contract 1.2.0 floor. The live plan compacts at 200,000 UTF-8 bytes and hard-stops at 240,000, leaving safety margin below the API ceiling. Exact archive hashing, unresolved-inline preservation, pre/post open-record set equality, current-first layout, and post-write readback are mandatory. |
| `readiness_notification_order` | `docs/OPERATING_POLICY.md` "universal reviewer-request ordering is strict: provider-confirmed ready/non-draft first, then refreshed post-ready gates, then and only then reviewer notification" | Order is fixed; reordering, or dropping the required middle `refreshed_post_ready_gates` step, is **breaking**. The validator checks the full ordered sequence (not just first/last elements) — see `fixtures/reordered_notification_contract.json`. |
| `escalation_classes.human_reserved[].includes` | `docs/OPERATING_POLICY.md` "Destructive includes deletion, reset/clean/discard..." / "Security includes secret/credential disclosure..." | Additive is **minor**; removing an item from `includes` without also removing it from the `PROMPT.md` prose is **breaking** (and would itself require a Human-directed decision, per the escalation-classes rule). |
| `escalation_classes.label_alone_does_not_escalate` | `docs/OPERATING_POLICY.md` "Labels such as production, protected/release branch, cost, or external communication do not by themselves create a second approval principal" | Advisory list; additive-only. |
| `done_integrity.required_proof`, `.receipt_fields`, `.merged_pr_or_done_placement_alone_is_not_proof` | `docs/OPERATING_POLICY.md` "Before any move to Done, explicitly prove..." and the terminal-receipt paragraph | Mirrors `docs/FILESYSTEM_DOCKER_CONTRACT.md`'s audit-trail convention (name the exact fields, never an aggregate claim). `merged_pr_or_done_placement_alone_is_not_proof` must stay `true` — a merged PR or Done-column placement alone is never sufficient; enforced by the validator's `missing_required_invariant` check, see `fixtures/false_done_placement_alone_not_proof_contract.json`. `required_proof` must keep `canonical_merged_identity_and_accepted_head` (see `fixtures/missing_canonical_merged_identity_done_proof_contract.json` — this is the base head-identity proof every other `required_proof` entry checks a property of) and `no_unique_local_or_untracked_work` (see `fixtures/missing_no_unique_local_done_proof_contract.json`), and `receipt_fields` must keep `local_head` (see `fixtures/missing_local_head_done_receipt_contract.json`) — the receipt field is what the proof is checked against, so all are independently enforced. |
...
**Precedent**: `contract_version` `1.0.0` → `1.1.0` (this repository's first
bump) added `queue_claim_identity.routine_identity_components` /
`.routine_identity_excludes_sender_ids` / `.cross_sender_coalescing_permitted`
/ `.coalescing_preserved_state` and
`worker_helper_receipts.routine_wake_coalescing_receipt_fields` — five new
fields, all newly-mandatory-but-additive (no existing field removed, weakened,
or reordered), so it is **MINOR**, not MAJOR. `source_charter_effective_version`
was left at `2026-09-03a` because no `PROMPT.md` rule changed; the new floors
are a contract/RFC-level clarification only, recorded in
`docs/DECISIONS.md#cross-sender-routine-wake-coalescing-is-identity-based-not-sender-based-2026-09-03`.
Because `validate_plugin_snapshot()` requires exact `plugin_contract_version`
equality (not merely `>= min_supported`), a plugin still vendored at `1.0.0`
fails closed against this `1.1.0` contract until it re-vendors — consistent
with §4 point 3's "older deployed plugin fails observably" rule, even though
the bump itself is semantically backward-compatible.
