...
operator procedure that copies complete source files or a compiled aggregate
can overwrite unrelated behavior that differs between the branch base and the
currently deployed application. Reviewing only the task diff cannot reveal that
blast radius.

Before calling a manual deployment safe or category-only, style-only, or
otherwise isolated:

1. Read the actual deployment command, script, or checklist and enumerate every
   whole file, generated bundle, configuration file, and install target it
   replaces.
2. For each replaced path, compare the candidate payload with both its declared
   base and the currently deployed version. Treat unexplained production-only
   markup, styles, configuration, or generated bytes as behavior at risk even
   when the task did not introduce the difference.
3. Trace generated aggregates back to their source inputs. Rebuild them only
   from the reconciled source; copying a fresh bundle compiled from a divergent
   branch merely makes the overwrite reproducible.
4. Preserve intended deployed behavior by either reconciling it into the task
   branch or applying the minimal feature change to the deployed source line and
   rebuilding its artifacts. Do not improvise edits directly in production.
5. Run fresh Review and QA against the exact payload that will be copied. Record
   the deployed-version comparison and the complete replacement set in the
   handoff.

The safety question is therefore "what bytes will this procedure replace?",
not only "what lines did this task author?" Whole-file deployment transfers the
candidate file's cumulative history, including divergence unrelated to the
feature.
